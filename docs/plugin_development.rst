...
To implement the required code, we will edit the ``confpage.py`` file generated by cokkiecutter as follows:
...
Most of it corresponds to the usual code of Qt widgets-based user interfaces.
In this case, our options section corresponds to a ``QGroupBox``, where the parameters are organized vertically using a ``QVBoxLayout``.
